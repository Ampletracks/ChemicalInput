...
const ChemicalInput = class {
  ...
  constructor(_el) {
    ...
    const dom = `<div class="chemical-input">
      ...
      <div class="chemical-input__actions">
        <div class="chemical-input__actions__faves-wrap">
          ...
          <button data-action="delete">Delete favourite</button>
        </div>
      </div>
    </div>`;
    ...
    this.actionDeleteBtn = this.actionsEl.querySelector('button[data-action="delete"]');
    ...
    this.actionDeleteBtn.addEventListener('click', this.deleteFave.bind(this));
  ...
  parseInput() {
    this.chainData = stringToJson(this.chainInput.value);

    // Set mode?
    const regex = /([a-z]{1,2}\s*[0-9\.]+[,\s]?|(?:atomic|weight|formula|at|wt))/gi;
    const chemArray = this.chainInput.value.match(regex);
  ...
  renderChain() {
    ...
    this.chainInput.value = jsonToString(this.chainData, this.mode);
  }
  ...
  initActionsDropdown(opts) {
    if (opts) {
      this.favesDropdown.innerHTML = opts;
    }
    this.favesDropdown.value = `init`;
    this.favesDropdown.setAttribute('data-selection', ``);
  }

  selectFave() {
    this.chainData = stringToJson(this.favesDropdown.value);
    this.renderChain();
  }

  // Enables naming of the new fave before saving.
  saveFaveAs() {
    // Use formula string as temp name.
    this.favesName.value = jsonToString(this.chainData, this.mode);
    this.favesName.focus();
    this.favesDropdown.setAttribute('data-selection', this.favesDropdown.value); 
  }

  // Sends the fave to the API.
  saveFave() {
    const data = {
      "name": this.favesName.value,
      "data": this.chainInput.value
    }

    console.log('Send current chain to the saveFavourite endpoint.', data);
    if (confirm('Send current chain to the saveFavourite endpoint.\n\n'+data)) {
      this.initActionsDropdown();

      // @TODO: Add new fave to faves array.
      // Would it be easier for endpoint to return updated array upon success or fail?
    }
  }

  deleteFave() {
    alert('Send selected favourite to the deleteFavourite endpoint.\n\n'+this.favesDropdown.value);
  }
  ...
  onActionChange() {
    switch(this.favesDropdown.value) {
      ...
      default: 
        this.favesDropdown.setAttribute('data-selection', 'compound'); 
        this.selectFave();
    }
  }
  ...
  add(inputs) {
    if (Array.isArray(inputs)) {
      [...inputs].forEach(input => {
        const el = new ChemicalInput(input);
        this.items.push(el);
      });
    } else {
      const el = new ChemicalInput(inputs);
      this.items.push(el);
    }
  }

  async getFaves() {
    this.faves = await fetch(`../data/chemical-input-faves.json`).then(response => response.json());
    this.initFaves();
  }

  initFaves() {
    let opts = `<option value="init">Select action...</option>`;
    opts += `<option value="clear">Clear chain</option>`;
    opts += `<option value="save-as">Save chain as...</option>`;
    this.faves.forEach(fave => {
      opts += `<option value="${fave.data}" data-id="${fave.id}">${fave.name}</option>`;
    });

    this.items.forEach(item => {
      item.initActionsDropdown(opts);
    });
  }
}
...
const stringToJson = (string) => {
  const regex = /([a-z]{1,2}\s*[0-9\.]+[,\s]?|(?:atomic|weight|formula|at|wt))/gi;
  const chemArray = string.match(regex);
  ...
  return chemArray.map(a => {
    const symbol = a.replace(/[^a-zA-Z]+/g, '');
    const elemFromPeriodicTable = json.find(e => e.symbol == symbol);
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  let myInputs = new ChemicalInputs({
    favouritesSaveHandler: function(favouriteName, formulaString) {
      console.log(favouriteName, formulaString);
